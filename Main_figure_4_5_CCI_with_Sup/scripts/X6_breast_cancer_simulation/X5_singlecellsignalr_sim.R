# Runs SingleCellSignalR on the simulated spatial data.
#
...
################################################################################
                    # Environment setup #
################################################################################
# TODO update to your working directory
work_dir <- "/Users/uqbbalde/Desktop/Uni_Studies/projects/stLearn/"
setwd(work_dir)

# TODO update to your conda env
...
breast_ad <- sc$read_h5ad(paste0(data_dir, 'spatialsim_v2.h5ad'))
...
for (i in 1:n_ccis) {
  cci <- all_ccis[i]
  cell1 <- str_split(cci, '-')[[1]][1]
  cell2 <- str_split(cci, '-')[[1]][2]
  lr_df <- signal[[i]]
  n_lrs <- dim(lr_df)[1]
  int_df[cell1, cell2] <- n_lrs
}
